...
const EMAIL_REGEXP = /[A-Z0-9._%+-]+@[A-Z0-9-]+.+.[A-Z]{2,4}/igm;
...
export const NewsLetter = () => {
  ...
  const sendEmail = (e) => {
    e.preventDefault();
    // emailjs.sendForm(process.env.REACT_APP_SERVICE_ID, process.env.REACT_APP_TEMPLATE_ID, e.target,
      ...
      <div className='newsletters__content'>
        ...
        <div className='newsletters__form-block'>
          <form className='newsletters__form' onSubmit={sendEmail}>
            <input ref={INPUT} type="email" name="mail" id="newslettersInput" placeholder='Your e-mail...' className='newsletters__input' onChange={updateInput}/>
            <button type='submit' className='newsletters__button'>Subscribe</button>
          </form>
          <div className={`newsletters__confirm_wrap ${isVisible ? "newsletters__confirm_wrap_visible" : ""} ${isVisible === 1 ? 'icon-spinner' : 'icon-check'}`}>
            <span className='newsletters__confirm'>{isVisible === 1 ? 'Sending' : 'Successfully sent!'}</span>
          </div>
        </div>
      </div>
    </section>
